...
from eigensdk_python.crypto.bls.attestation import G1Point, G2Point, Signature
import eigensdk_python.crypto.bls.attestation as bls
from eigensdk_python.services.avsregistry.avsregistry import AvsRegistryService, OperatorAvsState, QuorumAvsState, SignedTaskResponseDigest, CallOpts
from eigensdk_python.utils import exeption_to_dict
import asyncio
import json
...
class BlsAggregationServiceResponse:
	...
	def to_json(self, indent=None) -> str:
		...
		return json.dumps({
			...
			"signers_apk_g2": {
				"X": [self.signers_apk_g2.getX().get_a().getStr(16).decode("utf-8"), self.signers_apk_g2.getX().get_b().getStr(16).decode("utf-8")],
				"Y": [self.signers_apk_g2.getY().get_a().getStr(16).decode("utf-8"), self.signers_apk_g2.getY().get_b().getStr(16).decode("utf-8")]
			},
			"signers_agg_sig_g1": {
...
class BlsAggregationService(BlsAggregationServiceInterface):
	...
	async def initialize_new_task(
		...
		operators_avs_state_dict = await self.avs_registry_service.get_operators_avs_state_at_block(quorum_numbers, task_created_block)
		quorums_avs_state_dict = await self.avs_registry_service.get_quorums_avs_state_at_block(quorum_numbers, task_created_block)
	...
	async def process_new_signature(
		...
		if task_response_digest not in cd.aggregated_operators_dict:
			digest_aggregated_operators:AggregatedOperators = AggregatedOperators(
				signers_apk_g2 = bls.new_zero_g2_point() + operators_avs_state_dict[operator_id].operator_info.pub_keys.g2_pub_key,
				signers_agg_sig_g1=bls_sign,
				signers_operator_ids_set={operator_id: True},
				signers_total_stake_per_quorum=operators_avs_state_dict[operator_id].stake_per_quorum
			)
		else:
			digest_aggregated_operators:AggregatedOperators = cd.aggregated_operators_dict[task_response_digest]

			digest_aggregated_operators.signers_agg_sig_g1 = digest_aggregated_operators.signers_agg_sig_g1 + bls_sign
			digest_aggregated_operators.signers_apk_g2 = digest_aggregated_operators.signers_apk_g2 + operators_avs_state_dict[operator_id].operator_info.pub_keys.g2_pub_key
			digest_aggregated_operators.signers_operator_ids_set[operator_id] = True
			for quorum_num, stake_amount in operators_avs_state_dict[operator_id].stake_per_quorum.items():
				...
				digest_aggregated_operators.signers_total_stake_per_quorum[quorum_num] += stake_amount
		self.aggregated_responses[task_index].aggregated_operators_dict[task_response_digest] = digest_aggregated_operators
	...
	async def get_aggregated_response(self, task_index: int):
		...
		try:
			...
			return BlsAggregationServiceResponse(err=asyncio.TimeoutError(f"task {task_index} expired")), None
	...
	def __stake_thresholds_met(
		...
		for quorum_num, quorum_threshold_percentage in quorum_threshold_percentages_map.items():
	...
	def __verify_signature(
		...
		operator_g2_pub_key = operators_avs_state_dict[operator_id].operator_info.pub_keys.g2_pub_key
		if not operator_g2_pub_key:
			return ValueError(f"TaskId: {task_index} operator G2 PubKey not fount for operator {operator_id}")
