import asyncio
import unittest
from eigensdk_python.crypto.bls.attestation import KeyPair, Signature, new_g1_point, new_zero_g1_point
from .blsagg import BlsAggregationService, BlsAggregationServiceResponse
from eigensdk_python.services.avsregistry.avsregistry_fake import FakeAvsRegistryService, TestOperator
...
class TestBlsAggregationService(unittest.IsolatedAsyncioTestCase):
	...
	async def test_case_1(self):
		...
		asyncio.create_task(bls_aggregation_service.process_new_signature(
			task_index=task_index, 
		...
		got_aggregated_response, err = await bls_aggregation_service.get_aggregated_response(task_index)
	...
	async def test_case_2(self):
		...
		fake_avs_registry_service = FakeAvsRegistryService(block_number, [operator_1, operator_2, operator_3])

		bls_aggregation_service = BlsAggregationService(fake_avs_registry_service, hash_function)

		await bls_aggregation_service.initialize_new_task(task_index, block_number, quorum_numbers, quorum_threshold_percentages, self.time_to_expire_task)

		sign_1 = operator_1.bls_key_pair.sign_message(task_response_digest);
		asyncio.create_task(bls_aggregation_service.process_new_signature(task_index, task_response, sign_1, operator_1.operator_id))

		sign_2 = operator_2.bls_key_pair.sign_message(task_response_digest);
		asyncio.create_task(bls_aggregation_service.process_new_signature(task_index, task_response, sign_2, operator_2.operator_id))

		sign_3 = operator_3.bls_key_pair.sign_message(task_response_digest);
		asyncio.create_task(bls_aggregation_service.process_new_signature(task_index, task_response, sign_3, operator_3.operator_id))
		...
		want_aggregated_response = BlsAggregationServiceResponse(
			...
			task_response_digest=task_response_digest,
			non_signers_pubkeys_g1=[],
			quorum_apks_g1=[operator_1.bls_key_pair.pub_g1 + operator_2.bls_key_pair.pub_g1 + operator_3.bls_key_pair.pub_g1],
			signers_apk_g2=operator_1.bls_key_pair.pub_g2 + operator_2.bls_key_pair.pub_g2 + operator_3.bls_key_pair.pub_g2,
	...
	async def test_case_4(self):
		...
		await bls_aggregation_service.initialize_new_task(task_index_1, block_number, quorum_numbers, quorum_threshold_percentages, self.time_to_expire_task)

		task_index_2 = 2
		task_response_2="sample text response for tast case 4.2"
		task_response_digest_2 = hash_function(task_response_2);
		await bls_aggregation_service.initialize_new_task(task_index_2, block_number, quorum_numbers, quorum_threshold_percentages, self.time_to_expire_task)

		task_1_sign_1 = operator_1.bls_key_pair.sign_message(task_response_digest_1);
		asyncio.create_task(run_delayed(1.0, bls_aggregation_service, "process_new_signature", task_index_1, task_response_1, task_1_sign_1, operator_1.operator_id))

		task_2_sign_1 = operator_1.bls_key_pair.sign_message(task_response_digest_2);
		asyncio.create_task(run_delayed(1.2, bls_aggregation_service, "process_new_signature", task_index_2, task_response_2, task_2_sign_1, operator_1.operator_id))

		task_1_sign_2 = operator_2.bls_key_pair.sign_message(task_response_digest_1);
		asyncio.create_task(run_delayed(1.3, bls_aggregation_service, "process_new_signature", task_index_1, task_response_1, task_1_sign_2, operator_2.operator_id))

		task_2_sign_2 = operator_2.bls_key_pair.sign_message(task_response_digest_2);
		asyncio.create_task(run_delayed(1.2, bls_aggregation_service, "process_new_signature", task_index_2, task_response_2, task_2_sign_2, operator_2.operator_id))
		...
		got_aggregated_response_1, err = await bls_aggregation_service.get_aggregated_response(task_index_1)
		self.assertIsNone(err, f"No error expected.")
		got_aggregated_response_2, err = await bls_aggregation_service.get_aggregated_response(task_index_2)
		self.assertIsNone(err, f"No error expected.")
	...
	async def test_case_6(self):
		...
		asyncio.create_task(run_delayed(1.5, bls_aggregation_service, "process_new_signature", task_index, task_response, bls_sign, operator_1.operator_id))
