...
def test_register_operator():
    ...
    try:
        ...
        operator_ecdsa_private_key = ecdsa.SigningKey.from_string(
            bytes.fromhex(config["operator_private_key"].replace("0x", "")),
...
def test_remove_strategies():
    ...
    try:
        ...
        receipt = clients.avs_writer.remove_strategies(
            quorum_number,
            indices_to_remove
        )
...
def test_create_operator_directed_avs_rewards_submission():
    ...
    try:
        ...
        assert receipt is not None
        assert receipt['status'] == 1
        print(f"Created operator-directed AVS rewards submission with tx hash: {receipt['transactionHash'].hex()}")
