...
class TestELWriter(unittest.TestCase):
    ...
    def test_process_claim(self, mock_send_transaction):
        ...
        expected_claim_tuple = (
            ...
            [b"proof2", b"proof3"],
            [
                (Web3.to_checksum_address("0x2b961e3959b79326a8e7f64ef0d2d825707669b5"), 1000000000000000000),
                (Web3.to_checksum_address("0x2b961e3959b79326a8e7f64ef0d2d825707669b5"), 2000000000000000000)
            ]
        )
        mock_receipt = MagicMock(spec=TxReceipt)
        mock_send_transaction.return_value = mock_receipt
    ...
    def test_modify_allocations(self, mock_send_transaction):
        ...
        operator_set_id = 42
        strategies = ["0x3c44cdddb6a900fa2b585dd299e03d12fa4293bc", "0x90f79bf6eb2c4f870365e785982e1f101e93b906"]
    ...
    def test_deregister_from_operator_sets(self, mock_send_transaction):
        ...
        mock_function = MagicMock()
        self.mock_allocation_manager.functions.deregisterFromOperatorSets.return_value = mock_function
        mock_receipt = MagicMock(spec=TxReceipt)
        mock_send_transaction.return_value = mock_receipt
        result = self.el_writer.deregister_from_operator_sets(operator_address, request)
        self.mock_allocation_manager.functions.deregisterFromOperatorSets.assert_called_once_with(expected_params)
        mock_send_transaction.assert_called_once_with(
            mock_function,
            self.mock_pk_wallet,
            self.mock_eth_http_client
