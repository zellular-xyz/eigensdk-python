...
class TestAvsRegistryWriter(unittest.TestCase):
    ...
    def test_register_operator(
        self, mock_urandom, mock_send_transaction, mock_convert, mock_bn254_point, mock_account
    ):
        # Set up test data
        ...
        self.mock_registry_coordinator.functions.pubkeyRegistrationMessageHash.return_value.call.return_value = (
            mock_g1_hashed_msg
        )
        ...
        self.mock_el_reader.calculate_operator_avs_registration_digest_hash.return_value = (
            mock_digest_hash
        )
        ...
        result = self.avs_writer.register_operator(
            mock_operator_ecdsa_private_key, mock_bls_key_pair, quorum_numbers, socket
        )
        ...
        mock_send_transaction.assert_called_once_with(
            mock_register_func, self.mock_pk_wallet, self.mock_eth_http_client
        )
    ...
    def test_register_operator_in_quorum_with_avs_registry_coordinator(
        self, mock_send_transaction, mock_convert, mock_bn254_point, mock_account
    ):
        # Set up test data
    ...
    def test_update_stakes_of_entire_operator_set_for_quorums(
        self, mock_nums_to_bytes, mock_send_transaction
    ):
        # Setup test data
        ...
        self.mock_registry_coordinator.functions.updateOperatorsForQuorum.return_value = (
            mock_function
        )
        ...
        self.mock_registry_coordinator.functions.updateOperatorsForQuorum.assert_called_once_with(
            operators_per_quorum, b"quorum_numbers_bytes"
        )
    ...
    def test_register_operator_with_churn(
        ...
        mock_bls_key_pair = MagicMock(spec=KeyPair)
        mock_g1_point = MagicMock()
        mock_g1_point.get_operator_id.return_value = (
        ...
        self.mock_registry_coordinator.functions.calculateOperatorChurnApprovalDigestHash.return_value.call.return_value = (
            mock_churn_digest
        )
        ...
        self.mock_web3.eth.account.sign_message.side_effect = [
            mock_operator_sig,
            mock_churn_approval_sig,
        ]

        # Mock registerOperatorWithChurn function
        mock_register_func = MagicMock()
        self.mock_registry_coordinator.functions.registerOperatorWithChurn.return_value = (
            mock_register_func
        )
    ...
    def test_deregister_operator(self, mock_send_transaction):
        ...
        # Verify interactions
        self.mock_registry_coordinator.functions.deregisterOperator.assert_called_once_with(
            quorum_numbers
        )
    ...
    def test_create_slashable_stake_quorum(self, mock_send_transaction):
        ...
        strategy_params = [
            {"strategy": "0x3c44cdddb6a900fa2b585dd299e03d12fa4293bc", "multiplier": 10000},
            {"strategy": "0x90f79bf6eb2c4f870365e785982e1f101e93b906", "multiplier": 20000},
        ]
        ...
        mock_function = MagicMock()
        self.mock_registry_coordinator.functions.createSlashableStakeQuorum.return_value = (
            mock_function
        )
