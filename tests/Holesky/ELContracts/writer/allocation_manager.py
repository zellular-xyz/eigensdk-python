...
def test_modify_allocations():
    ...
    register_tx = holesky_el_writer.register_for_operator_sets(
        registry_coordinator_addr=registry_coordinator_addr, request=register_request
    )
    ...
    return holesky_el_writer.modify_allocations(
        operator_address=operator_address, allocations=allocations, wait_for_receipt=True
    )
...
def test_set_allocation_delay():
    ...
    return holesky_el_writer.set_allocation_delay(
        operator_address=operator_address, delay=delay, wait_for_receipt=False
    )
...
def test_deregister_from_operator_sets():
    ...
    return holesky_el_writer.deregister_from_operator_sets(operator=operator, request=request)
...
def test_register_for_operator_sets():
    ...
    return holesky_el_writer.register_for_operator_sets(
        registry_coordinator_addr=registry_coordinator_addr, request=request
    )
