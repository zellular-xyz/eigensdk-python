...
class ELReader:
    ...
    def get_strategies_for_operator_set(self, operator_set: dict) -> List[Address]:
        ...
        set_id = int(operator_set["Id"])  # Convert ID to int for uint32
        avs_address = Web3.to_checksum_address(operator_set["Avs"])  # Convert to checksum address
    ...
    def get_delegated_operator(
        ...
        # Convert the returned hex string to bytes
        return bytes.fromhex(delegated_operator[2:])  # Remove '0x' prefix before converting

    @typechecked
    def get_operator_details(self, operator: Dict[str, Any]) -> Dict[str, Any]:

        operator_address = Web3.to_checksum_address(operator["Address"])
    ...
    def list_admins(self, account_address: Address) -> List[Address]:
        try:
            ...
            return [
                bytes.fromhex(addr[2:]) for addr in admins
    ...
    def get_rewards_updater(self) -> Address:
        ...
        # Convert the returned hex string to bytes
        return bytes.fromhex(rewards_updater[2:])  # Remove '0x' prefix before converting
    ...
    def get_claimer_for(self, earner: Address) -> Address:
        ...
        # Convert the returned hex string to bytes
        return bytes.fromhex(claimer_address[2:])  # Remove '0x' prefix before converting
    ...
    def get_strategy_and_underlying_token(
        ...
        return strategy_contract, underlying_token_addr

    @typechecked
    def get_strategy_and_underlying_erc20_token(
        self, strategy_addr: Address  # Change from str to Address
    ...
    def calculate_operator_avs_registration_digest_hash(
        self,
        operator: Address,  # Change from str to Address
        avs: Address,  # Change from str to Address
        salt: bytes,
        expiry: int,
    ) -> bytes:
        digest_hash = self.avs_directory.functions.calculateOperatorAVSRegistrationDigestHash(
    ...
    def get_genesis_rewards_timestamp(
        ...
        return genesis_rewards_timestamp
    ...
    def get_num_operator_sets_for_operator(
        self, operator_address: Address  # Change type annotation from str to Address
    ) -> int:
    ...
    def get_slashable_shares(
        ...
        if avs_address is None:
            return None  # Just return None instead of tuple with error
    ...
    def get_slashable_shares_for_operator_sets_before(
        ...
        for operator_set in operator_sets:
            ...
            if "Id" not in operator_set or "Avs" not in operator_set:
                return None, ValueError(f"Invalid operator_set format: {operator_set}")
            set_id = int(operator_set["Id"])
            avs_address = operator_set["Avs"]

            if set_id == 0:
                return None, ValueError(f"Legacy AVSs not supported (operatorSet.Id == 0)")

            operators = self.get_operators_for_operator_set(operator_set)

            if not operators:
                return None, ValueError(f"Failed to get operators for OperatorSet: {operator_set}")

            strategies = self.get_strategies_for_operator_set(operator_set)

            if not strategies:
                return None, ValueError(f"Failed to get strategies for OperatorSet: {operator_set}")
